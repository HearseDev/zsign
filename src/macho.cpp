...
bool ZMachO::OpenFile (const char* szPath) {
    ...
    if (NULL != m_pBase) {
        ...
        if (FAT_CIGAM == magic || FAT_MAGIC == magic) {
            ...
            int nFatArch = (FAT_MAGIC == magic) ? pFatHeader->nfat_arch :
                                                  LE (pFatHeader->nfat_arch);
            for (int i = 0; i < nFatArch; i++) {
                fat_arch* pFatArch =
                (fat_arch*)(m_pBase + sizeof (fat_header) + sizeof (fat_arch) * i);
                uint8_t* pArchBase = m_pBase +
                ((FAT_MAGIC == magic) ? pFatArch->offset : LE (pFatArch->offset));
                uint32_t uArchLength =
                (FAT_MAGIC == magic) ? pFatArch->size : LE (pFatArch->size);
                if (!NewArchO (pArchBase, uArchLength)) {
                    ZLog::ErrorV (">>> Invalid Arch File In Fat Macho File!\n");
                    return false;
                }
            }
        } else if (MH_MAGIC == magic || MH_CIGAM == magic ||
        MH_MAGIC_64 == magic || MH_CIGAM_64 == magic) {
...
bool ZMachO::Sign (ZSignAsset* pSignAsset,
bool bForce,
string strBundleId,
...
const string& strCodeResourcesData) {
    ...
    for (size_t i = 0; i < m_arrArchOes.size (); i++) {
        ...
        if (!archo->Sign (pSignAsset, bForce, strBundleId, strInfoPlistSHA1,
            strInfoPlistSHA256, strCodeResourcesData)) {
            if (!archo->m_bEnoughSpace && !m_bCSRealloced) {
                m_bCSRealloced = true;
                if (ReallocCodeSignSpace ()) {
                    return Sign (pSignAsset, bForce, strBundleId,
                    strInfoPlistSHA1, strInfoPlistSHA256, strCodeResourcesData);
                }
            }
            return false;
        }
...
bool ZMachO::ReallocCodeSignSpace () {
    ...
    if (1 == m_arrArchOes.size ()) {
        ...
        uint32_t uFatHeaderSize =
        sizeof (fat_header) + arrArches.size () * sizeof (fat_arch);
        ...
        for (size_t i = 0; i < arrArches.size (); i++) {
            ...
            uint8_t* pData =
            (uint8_t*)MapFile (strNewArchOFile.c_str (), 0, 0, &sSize, true);
            if (NULL == pData) {
