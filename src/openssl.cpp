...
bool GetCMSContent (const string& strCMSDataInput, string& strContentOutput) {
    ...
    OPENSSL_assert ((size_t)BIO_write (in, strCMSDataInput.data (),
                    (int)strCMSDataInput.size ()) == strCMSDataInput.size ());
            ...
            txtobj)) { // V_ASN1_OCTET_STRING
                ...
                for (int m = 0; m < nCount; m++) {
                    ...
                    if (NULL != av) {
                        ...
                        transform (strSHASum.begin (), strSHASum.end (),
                        strSHASum.begin (), ::tolower);
                        jvOutput["attrs"]["CDHashes2"]["data"].push_back (strSHASum);
                    }
                }
            } else if (0 ==
            strcmp ("1.2.840.113635.100.9.1",
            txtobj)) { // V_ASN1_OCTET_STRING
                ASN1_TYPE* av = X509_ATTRIBUTE_get0_type (attr, 0);
                if (NULL != av) {
                    string strPList;
...
const string& strPassword) {
    ...
    if (NULL != bioPKey) {
        evpPKey =
        PEM_read_bio_PrivateKey (bioPKey, NULL, NULL, (void*)strPassword.c_str ());
        if (NULL == evpPKey) {
            BIO_reset (bioPKey);
...
string& strCMSOutput) {
    ...
    strCDHashData, strCDHashesPlist, strCodeDirectorySlotSHA1,
