...
bool _GenerateCMS(X509 *scert, EVP_PKEY *spkey, const string &strCDHashData,
  ...
  int addHashPlist = CMS_signed_add1_attr_by_OBJ(
      si, obj, 0x4, strCDHashesPlist.c_str(), (int)strCDHashesPlist.size());
  ...
  for (size_t i = 0; i < strAltnateCodeDirectorySlot256.size(); i++) {
    snprintf(buf, sizeof(buf), "%02x",
             (uint8_t)strAltnateCodeDirectorySlot256[i]);
    sha256 += buf;
  }
  ...
  X509_ATTRIBUTE_set1_data(attr, V_ASN1_SEQUENCE,
                           type_256->value.asn1_string->data,
...
bool GenerateCMS(const string &strSignerCertData,
                 const string &strSignerPKeyData, const string &strCDHashData,
                 const string &strCDHashesPlist, string &strCMSOutput) {
  BIO *bcert =
      BIO_new_mem_buf(strSignerCertData.c_str(), (int)strSignerCertData.size());
  BIO *bpkey =
      BIO_new_mem_buf(strSignerPKeyData.c_str(), (int)strSignerPKeyData.size());
  ...
  return ::_GenerateCMS(scert, spkey, strCDHashData, strCDHashesPlist, "", "",
                        strCMSOutput);
}
...
bool GetCMSContent(const string &strCMSDataInput, string &strContentOutput) {
  ...
  OPENSSL_assert((size_t)BIO_write(in, strCMSDataInput.data(),
                                   (int)strCMSDataInput.size()) ==
...
bool GetCertSubjectCN(X509 *cert, string &strSubjectCN) {
  ...
  X509_NAME_ENTRY *common_name_entry =
      X509_NAME_get_entry(name, common_name_loc);
  ...
  strSubjectCN.clear();
  strSubjectCN.append((const char *)common_name_asn1->data,
                      common_name_asn1->length);
...
#if OPENSSL_VERSION_NUMBER < 0x10100000L
  jvCertInfo["Validity"]["NotBefore"] =
      ASN1_TIMEtoString(X509_get_notBefore(cert));
  jvCertInfo["Validity"]["NotAfter"] =
      ASN1_TIMEtoString(X509_get_notAfter(cert));
#else
  jvCertInfo["Validity"]["NotBefore"] =
      ASN1_TIMEtoString(X509_get0_notBefore(cert));
  jvCertInfo["Validity"]["NotAfter"] =
      ASN1_TIMEtoString(X509_get0_notAfter(cert));
#endif
...
bool GetCMSInfo(uint8_t *pCMSData, uint32_t uCMSLength, JValue &jvOutput) {
  ...
  if (pos) {
    if ((*pos)) {
      ZBase64 b64;
      jvOutput["content"] =
          b64.Encode((const char *)(*pos)->data, (*pos)->length);
    }
  }
    ...
    for (int j = 0; j < nSignedAttsCount; j++) {
      ...
      if (0 == strcmp("1.2.840.113549.1.9.3", txtobj)) { // V_ASN1_OBJECT
        ...
        if (NULL != av) {
          ...
          jvOutput["attrs"]["ContentType"]["data"] =
              OBJ_nid2ln(OBJ_obj2nid(av->value.object));
        }
      } else if (0 == strcmp("1.2.840.113549.1.9.4",
                             txtobj)) { // V_ASN1_OCTET_STRING
        ASN1_TYPE *av = X509_ATTRIBUTE_get0_type(attr, 0);
        if (NULL != av) {
          string strSHASum;
          char buf[16] = {0};
          for (int m = 0; m < av->value.octet_string->length; m++) {
            snprintf(buf, sizeof(buf), "%02x",
                     (uint8_t)av->value.octet_string->data[m]);
            strSHASum += buf;
          }
          ...
          jvOutput["attrs"]["MessageDigest"]["data"] = strSHASum;
                 ...
                 strcmp("1.2.840.113549.1.9.5", txtobj)) { // V_ASN1_UTCTIME
        ASN1_TYPE *av = X509_ATTRIBUTE_get0_type(attr, 0);
        if (NULL != av) {
          ...
          jvOutput["attrs"]["SigningTime"]["data"] = strTime;
                 ...
                 strcmp("1.2.840.113635.100.9.2", txtobj)) { // V_ASN1_SEQUENCE
        ...
        for (int m = 0; m < nCount; m++) {
          ...
          if (NULL != av) {
            ...
            transform(strSHASum.begin(), strSHASum.end(), strSHASum.begin(),
                      ::tolower);
            jvOutput["attrs"]["CDHashes2"]["data"].push_back(strSHASum);
          }
        }
      } else if (0 == strcmp("1.2.840.113635.100.9.1",
                             txtobj)) { // V_ASN1_OCTET_STRING
        ASN1_TYPE *av = X509_ATTRIBUTE_get0_type(attr, 0);
        if (NULL != av) {
          string strPList;
          strPList.append((const char *)av->value.octet_string->data,
...
bool ZSignAsset::Init(const string &strSignerCertFile,
                      const string &strSignerPKeyFile,
                      const string &strProvisionFile,
                      const string &strEntitlementsFile,
  ...
  if (NULL != bioPKey) {
    evpPKey = PEM_read_bio_PrivateKey(bioPKey, NULL, NULL,
                                      (void *)strPassword.c_str());
    if (NULL == evpPKey) {
      BIO_reset(bioPKey);
      ...
      if (NULL == evpPKey) {
        ...
        if (NULL != p12) {
          if (0 == PKCS12_parse(p12, strPassword.c_str(), &evpPKey, &x509Cert,
                                NULL)) {
            CMSError();
          }
          PKCS12_free(p12);
  ...
  if (NULL == evpPKey) {
    ZLog::Error(">>> Can't Load P12 or PrivateKey File! Please Input The "
                "Correct File And Password!\n");
    return false;
  }
  ...
  if (NULL == x509Cert) {
    for (size_t i = 0; i < jvProv["DeveloperCertificates"].size(); i++) {
      ...
      BIO *bioCert =
          BIO_new_mem_buf(strCertData.c_str(), (int)strCertData.size());
      if (NULL != bioCert) {
        x509Cert = d2i_X509_bio(bioCert, NULL);
        if (NULL != x509Cert) {
          if (X509_check_private_key(x509Cert, evpPKey)) {
...
bool ZSignAsset::GenerateCMS(const string &strCDHashData,
                             const string &strCDHashesPlist,
                             const string &strCodeDirectorySlotSHA1,
                             const string &strAltnateCodeDirectorySlot256,
                             string &strCMSOutput) {
  return ::_GenerateCMS((X509 *)m_x509Cert, (EVP_PKEY *)m_evpPKey,
                        strCDHashData, strCDHashesPlist,
                        strCodeDirectorySlotSHA1,
                        strAltnateCodeDirectorySlot256, strCMSOutput);
}
