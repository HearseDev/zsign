...
const char* ZBase64::Encode (const char* szSrc, int nSrcLen) {
    ...
    if (i < nSrcLen) {
        ...
        p64[2] =
        rest > 1 ? GetB64char ((B1 (ulTmp) << 4 >> 2 | B2 (ulTmp) >> 6) & 0x3F) : '=';
...
const char* ZBase64::Decode (const char* szSrc, int nSrcLen, int* pDecLen) {
    ...
    for (i = 0; i < nSrcLen - 4; i += 4) {
        ...
        int b0 =
        (GetB64Index ((char)B0 (ulTmp)) << 2 | GetB64Index ((char)B1 (ulTmp)) << 2 >> 6) & 0xFF;
        int b1 =
        (GetB64Index ((char)B1 (ulTmp)) << 4 | GetB64Index ((char)B2 (ulTmp)) << 2 >> 4) & 0xFF;
        int b2 =
        (GetB64Index ((char)B2 (ulTmp)) << 6 | GetB64Index ((char)B3 (ulTmp)) << 2 >> 2) & 0xFF;
