...
bool JValue::isEmpty () const {
    switch (m_eType) {
    case E_NULL: return true; break;
    case E_INT: return (0 == m_Value.vInt64); break;
    case E_BOOL: return (false == m_Value.vBool); break;
    case E_FLOAT: return (0 == m_Value.vFloat); break;
    case E_ARRAY:
    case E_OBJECT: return (0 == size ()); break;
    case E_STRING: return (0 == strlen (asCString ()));
    case E_DATE: return (0 == m_Value.vDate); break;
    case E_DATA:
...
void JValue::CopyValue (const JValue& src) {
    ...
    switch (m_eType) {
    case E_ARRAY:
        m_Value.vArray = (NULL == src.m_Value.vArray) ?
        NULL :
        new vector<JValue> (*(src.m_Value.vArray));
        break;
    case E_OBJECT:
        m_Value.vObject = (NULL == src.m_Value.vObject) ?
        NULL :
        new map<string, JValue> (*(src.m_Value.vObject));
        break;
    case E_STRING:
        m_Value.vString =
        (NULL == src.m_Value.vString) ? NULL : NewString (src.m_Value.vString);
        break;
    case E_DATA: {
    ...
    default: m_Value = src.m_Value; break;
    }
}
...
int64_t JValue::asInt64 () const {
    switch (m_eType) {
    case E_INT: return m_Value.vInt64; break;
    case E_BOOL: return m_Value.vBool ? 1 : 0; break;
    case E_FLOAT: return int (m_Value.vFloat); break;
    case E_STRING: return _atoi64 (asCString ()); break;
    default: break;
    }
    return 0;
}
...
double JValue::asFloat () const {
    switch (m_eType) {
    case E_INT: return double (m_Value.vInt64); break;
    case E_BOOL: return m_Value.vBool ? 1.0 : 0.0; break;
    case E_FLOAT: return m_Value.vFloat; break;
    case E_STRING: return atof (asCString ()); break;
    default: break;
    }
    return 0.0;
}
...
bool JValue::asBool () const {
    switch (m_eType) {
    case E_BOOL: return m_Value.vBool; break;
    case E_INT: return (0 != m_Value.vInt64); break;
    case E_FLOAT: return (0.0 != m_Value.vFloat); break;
    case E_ARRAY:
    ...
    case E_STRING:
        ...
        break;
    case E_DATE: return (m_Value.vDate > 0); break;
    case E_DATA:
...
string JValue::asString () const {
    switch (m_eType) {
    case E_BOOL: return m_Value.vBool ? "true" : "false"; break;
    case E_INT: {
        char buf[256];
    ...
    case E_ARRAY: return "array"; break;
    case E_OBJECT: return "object"; break;
    case E_STRING:
        return (NULL == m_Value.vString) ? "" : m_Value.vString;
        break;
    case E_DATE: return "date"; break;
    case E_DATA: return "data"; break;
    default: break;
    }
    return "";
}
...
time_t JValue::asDate () const {
    switch (m_eType) {
    case E_DATE: return m_Value.vDate; break;
    case E_STRING: {
        if (isDateString ()) {
            tm ft = { 0 };
            sscanf (m_Value.vString + 5, "%04d-%02d-%02dT%02d:%02d:%02dZ", &ft.tm_year,
            &ft.tm_mon, &ft.tm_mday, &ft.tm_hour, &ft.tm_min, &ft.tm_sec);
            ft.tm_mon -= 1;
            ft.tm_year -= 1900;
            return mktime (&ft);
        }
...
bool JReader::readValue (JValue& jval) {
    ...
    readToken (token);
    switch (token.type) {
    case Token::E_True: jval = true; break;
    case Token::E_False: jval = false; break;
    case Token::E_Null: jval = JValue (); break;
    case Token::E_Number: return decodeNumber (token, jval); break;
    case Token::E_ArrayBegin: return readArray (jval); break;
    case Token::E_ObjectBegin: return readObject (jval); break;
    case Token::E_String: {
        string strval;
...
bool JReader::readToken (Token& token) {
    ...
    token.pbeg = m_pCur;
    switch (GetNextChar ()) {
    case '{': token.type = Token::E_ObjectBegin; break;
    case '}': token.type = Token::E_ObjectEnd; break;
    case '[': token.type = Token::E_ArrayBegin; break;
    case ']': token.type = Token::E_ArrayEnd; break;
    case ',': token.type = Token::E_ArraySeparator; break;
    case ':': token.type = Token::E_MemberSeparator; break;
    case 0: token.type = Token::E_End; break;
    case '"':
    ...
    case 'n':
        ...
        break;
    default: token.type = Token::E_Error; break;
    }
    token.pend = m_pCur;
    return true;
}
...
bool JReader::decodeNumber (Token& token, JValue& jval) {
    ...
    for (const char* p = pcur; p != token.pend; p++) {
        ...
        if ('.' == c || 'e' == c || 'E' == c) {
            ...
            return addError (
            "'" + string (token.pbeg, token.pend) + "' is not a number.", token.pbeg);
        } else {
            val = val * 10 + (c - '0');
        }
    }
...
bool JReader::decodeDouble (Token& token, JValue& jval) {
    ...
    return addError ("'" + string (token.pbeg, token.pend) + "' is too large or not a number.",
    token.pbeg);
}
    ...
    while (pcur != pend) {
        ...
        if ('\\' == c) {
            if (pcur != pend) {
                ...
                case 't': strdec += '\t'; break;
                case '/': strdec += '/'; break;
                case 'u': { // based on description from
...
void JReader::error (string& strmsg) const {
    ...
    char msg[64];
    snprintf (msg, sizeof (msg), "Error: Line %d, Column %d, ", row,
    int (m_pErr - plast) + 1);
    strmsg += msg + m_strErr + "\n";
}
...
void JWriter::FastWriteValue (const JValue& jval, string& strDoc) {
    switch (jval.type ()) {
    case JValue::E_NULL: strDoc += "null"; break;
    case JValue::E_INT: strDoc += v2s (jval.asInt64 ()); break;
    case JValue::E_BOOL: strDoc += jval.asBool () ? "true" : "false"; break;
    case JValue::E_FLOAT: strDoc += v2s (jval.asFloat ()); break;
    case JValue::E_STRING: strDoc += v2s (jval.asCString ()); break;
    case JValue::E_ARRAY: {
        strDoc += "[";
...
void JWriter::StyleWriteValue (const JValue& jval) {
    switch (jval.type ()) {
    case JValue::E_NULL: PushValue ("null"); break;
    case JValue::E_INT: PushValue (v2s (jval.asInt64 ())); break;
    case JValue::E_BOOL: PushValue (jval.asBool () ? "true" : "false"); break;
    case JValue::E_FLOAT: PushValue (v2s (jval.asFloat ())); break;
    case JValue::E_STRING: PushValue (v2s (jval.asCString ())); break;
    case JValue::E_ARRAY: StyleWriteArrayValue (jval); break;
    case JValue::E_OBJECT: {
        vector<string> arrKeys;
        jval.keys (arrKeys);
        if (!arrKeys.empty ()) {
...
string JWriter::d2s (time_t t) {
    ...
    char szDate[64] = { 0 };
    snprintf (szDate, sizeof (szDate), "%04d-%02d-%02dT%02d:%02d:%02dZ",
    ft.tm_year + 1900, ft.tm_mon + 1, ft.tm_mday, ft.tm_hour, ft.tm_min, ft.tm_sec);
    return szDate;
}
...
bool PReader::readValue (JValue& pval, Token& token) {
    switch (token.type) {
    case Token::E_True: pval = true; break;
    case Token::E_False: pval = false; break;
    case Token::E_Null: pval = JValue (); break;
    case Token::E_Integer: return decodeNumber (token, pval); break;
    case Token::E_Real: return decodeDouble (token, pval); break;
    case Token::E_ArrayNull: pval = JValue (JValue::E_ARRAY); break;
    case Token::E_ArrayBegin: return readArray (pval); break;
    case Token::E_DictionaryNull: pval = JValue (JValue::E_OBJECT); break;
    case Token::E_DictionaryBegin: return readDictionary (pval); break;
    case Token::E_Date: {
        string strval;
        decodeString (token, strval);

        tm ft = { 0 };
        sscanf (strval.c_str (), "%04d-%02d-%02dT%02d:%02d:%02dZ", &ft.tm_year,
        &ft.tm_mon, &ft.tm_mday, &ft.tm_hour, &ft.tm_min, &ft.tm_sec);
        ft.tm_mon -= 1;
        ft.tm_year -= 1900;
    ...
    default:
        return addError (
        "Syntax error: value, dictionary or array expected.", token.pbeg);
        break;
    }
    return true;
}
...
bool PReader::readToken (Token& token) {
    ...
    } else if ("<data/>" == label || "<date/>" == label ||
    "<string/>" == label || "<integer/>" == label || "<real/>" == label) {
...
bool PReader::readBinaryValue (const char*& pcur, JValue& pv) {
    ...
    case BPLIST_REAL: {
        ...
        switch (size) {
        case sizeof (float): pv = (double)(*(float*)buf);
        case sizeof (double): pv = (*(double*)buf); break;
        default: {
            assert (0);
            free (buf);
            return false;
    ...
    case BPLIST_UNK_0x70: {
        ...
        for (size_t i = 0; i < size; i++) {
            uint64_t uIndex =
            getUIntVal ((const char*)pcur + i * m_uDictParamSize, m_uDictParamSize);
            if (uIndex < m_uObjects) {
                const char* pval = (m_pBeg +
                getUIntVal (m_pOffsetTable + uIndex * m_uOffsetSize, m_uOffsetSize));
    ...
    case BPLIST_DICT: {
        ...
        for (size_t i = 0; i < size; i++) {
            ...
            uint64_t uKeyIndex =
            getUIntVal ((const char*)pcur + i * m_uDictParamSize, m_uDictParamSize);
            uint64_t uValIndex = getUIntVal (
            (const char*)pcur + (i + size) * m_uDictParamSize, m_uDictParamSize);

            if (uKeyIndex < m_uObjects) {
                const char* pval = (m_pBeg +
                getUIntVal (m_pOffsetTable + uKeyIndex * m_uOffsetSize, m_uOffsetSize));
                readBinaryValue (pval, pvKey);
            }

            if (uValIndex < m_uObjects) {
                const char* pval = (m_pBeg +
                getUIntVal (m_pOffsetTable + uValIndex * m_uOffsetSize, m_uOffsetSize));
