...
const struct option options[] = {
    ...
    {"remove_prov", no_argument, NULL, 'x'},
...
int main(int argc, char *argv[]) {
  ...
  int opt = 0;
  int argslot = -1;
  while (-1 != (opt = getopt_long(argc, argv, "dfvhc:k:m:o:ip:e:b:n:z:ql:wx",
                                  options, &argslot))) {
    switch (opt) {
    case 'd':
  ...
  bool bRet = bundle.SignFolder(&zSignAsset, strFolder, strBundleId,
                                strBundleVersion, strDisplayName, strDyLibFile,
                                bForce, bWeakInject, bEnableCache, bRemoveProv);
