...
const struct option options[] = { { "debug", no_argument, NULL, 'd' },
    ...
    { "cert", required_argument, NULL, 'c' }, { "pkey", required_argument, NULL, 'k' },
    { "prov", required_argument, NULL, 'm' }, { "remove_prov", no_argument, NULL, 'x' },
...
int usage () {
    ...
    ZLog::Print ("-x, --remove_prov\tRemove mobile provisioning profile.\n");
    ZLog::Print (
...
int main (int argc, char* argv[]) {
    ...
    int argslot = -1;
    while (-1 !=
    (opt = getopt_long (argc, argv, "dfvhc:k:m:o:ip:e:b:n:z:ql:wx", options, &argslot))) {
        switch (opt) {
    ...
    strDisplayName, strDyLibFile, bForce, bWeakInject, bEnableCache, bRemoveProv);
    ...
    if (!strOutputFile.empty ()) {
        ...
        if (NULL != getcwd (szOldFolder, PATH_MAX)) {
            if (0 == chdir (strBaseFolder.c_str ())) {
                ...
                SystemExec (
                "zip -q -%u -r '%s' Payload", uZipLevel, strOutputFile.c_str ());
                chdir (szOldFolder);
