...
bool ZArchO::Init (uint8_t* pBase, uint32_t uLength) {
    ...
    if (MH_MAGIC != m_pHeader->magic && MH_CIGAM != m_pHeader->magic &&
    MH_MAGIC_64 != m_pHeader->magic && MH_CIGAM_64 != m_pHeader->magic) {
        return false;
    }

    m_b64 = (MH_MAGIC_64 == m_pHeader->magic || MH_CIGAM_64 == m_pHeader->magic) ? true : false;
    m_bBigEndian =
    (MH_CIGAM == m_pHeader->magic || MH_CIGAM_64 == m_pHeader->magic) ? true : false;
    ...
    for (uint32_t i = 0; i < BO (m_pHeader->ncmds); i++) {
        ...
        case LC_SEGMENT: {
            ...
            if (0 == strcmp ("__TEXT", seglc->segname)) {
                ...
                for (uint32_t j = 0; j < BO (seglc->nsects); j++) {
                    section* sect = (section*)((pLoadCommand + sizeof (segment_command)) +
                    sizeof (section) * j);
                    if (0 == strcmp ("__text", sect->sectname)) {
                        if (BO (sect->offset) > (BO (m_pHeader->sizeofcmds) + m_uHeaderSize)) {
                            m_uLoadCommandsFreeSpace = BO (sect->offset) -
                            BO (m_pHeader->sizeofcmds) - m_uHeaderSize;
                        }
                    } else if (0 == strcmp ("__info_plist", sect->sectname)) {
                        m_strInfoPlist.append (
                        (const char*)m_pBase + BO (sect->offset), BO (sect->size));
                    }
                }
        ...
        case LC_SEGMENT_64: {
            ...
            if (0 == strcmp ("__TEXT", seglc->segname)) {
                ...
                for (uint32_t j = 0; j < BO (seglc->nsects); j++) {
                    section_64* sect =
                    (section_64*)((pLoadCommand + sizeof (segment_command_64)) +
                    sizeof (section_64) * j);
                    if (0 == strcmp ("__text", sect->sectname)) {
                        if (BO (sect->offset) > (BO (m_pHeader->sizeofcmds) + m_uHeaderSize)) {
                            m_uLoadCommandsFreeSpace = BO (sect->offset) -
...
const char* ZArchO::GetArch (int cpuType, int cpuSubType) {
    ...
    case CPU_TYPE_ARM64: {
        switch (cpuSubType) {
        case CPU_SUBTYPE_ARM64_ALL: return "arm64"; break;
        case CPU_SUBTYPE_ARM64_V8: return "arm64v8"; break;
        case 2: return "arm64e"; break;
        }
    } break;
    case CPU_TYPE_ARM64_32: {
        switch (cpuSubType) {
        case CPU_SUBTYPE_ARM64_ALL: return "arm64_32"; break;
        case CPU_SUBTYPE_ARM64_32_V8: return "arm64e_32"; break;
        }
    } break;
    case CPU_TYPE_X86: {
        switch (cpuSubType) {
        default: return "x86_32"; break;
        }
    } break;
    case CPU_TYPE_X86_64: {
        switch (cpuSubType) {
        default: return "x86_64"; break;
        }
    } break;
    }
    return "unknown";
...
const char* ZArchO::GetFileType (uint32_t uFileType) {
    switch (uFileType) {
    case MH_OBJECT: return "MH_OBJECT"; break;
    case MH_EXECUTE: return "MH_EXECUTE"; break;
    case MH_FVMLIB: return "MH_FVMLIB"; break;
    case MH_CORE: return "MH_CORE"; break;
    case MH_PRELOAD: return "MH_PRELOAD"; break;
    case MH_DYLIB: return "MH_DYLIB"; break;
    case MH_DYLINKER: return "MH_DYLINKER"; break;
    case MH_BUNDLE: return "MH_BUNDLE"; break;
    case MH_DYLIB_STUB: return "MH_DYLIB_STUB"; break;
    case MH_DSYM: return "MH_DSYM"; break;
    case MH_KEXT_BUNDLE: return "MH_KEXT_BUNDLE"; break;
    }
    return "MH_UNKNOWN";
}
...
void ZArchO::PrintInfo () {
    ...
    FormatSize (m_uCodeLength).c_str ());
    ZLog::PrintV ("\tSignLength: \t%d (%s)\n", m_uSignLength,
    FormatSize (m_uSignLength).c_str ());
    ...
    for (uint32_t i = 0; i < BO (m_pHeader->ncmds); i++) {
        ...
        if (LC_LOAD_DYLIB == BO (plc->cmd)) {
            ...
            const char* szDyLib =
            (const char*)(pLoadCommand + BO (dlc->dylib.name.offset));
            ZLog::PrintV ("\t\t\t%s\n", szDyLib);
...
const string& strInfoPlistSHA1,
const string& strInfoPlistSHA256,
...
string& strOutput) {
    ...
    SlotBuildEntitlements (IsExecute () ? pSignAsset->m_strEntitlementsData : strEmptyEntitlements,
    strEntitlementsSlot);
    SlotBuildDerEntitlements (
    IsExecute () ? pSignAsset->m_strEntitlementsData : "", strDerEntitlementsSlot);
    ...
    if (!bForce) {
        GetCodeSignatureExistsCodeSlotsData (m_pSignBase, pCodeSlots1Data,
        uCodeSlots1DataLength, pCodeSlots256Data, uCodeSlots256DataLength);
    }
    ...
    SlotBuildCodeDirectory (false, m_pBase, m_uCodeLength, pCodeSlots1Data,
    ...
    strCodeResourcesSHA1, strEntitlementsSlotSHA1, strDerEntitlementsSlotSHA1,
    IsExecute (), strCodeDirectorySlot);
    SlotBuildCodeDirectory (true, m_pBase, m_uCodeLength, pCodeSlots256Data,
    uCodeSlots256DataLength, execSegLimit, execSegFlags, strBundleId,
    pSignAsset->m_strTeamId, strInfoPlistSHA256, strRequirementsSlotSHA256,
    strCodeResourcesSHA256, strEntitlementsSlotSHA256,
    strDerEntitlementsSlotSHA256, IsExecute (), strAltnateCodeDirectorySlot);
    SlotBuildCMSSignature (pSignAsset, strCodeDirectorySlot,
    strAltnateCodeDirectorySlot, strCMSSignatureSlot);
    ...
    uint32_t uAltnateCodeDirectorySlotLength =
    (uint32_t)strAltnateCodeDirectorySlot.size ();
    ...
    uint32_t uSuperBlobHeaderLength =
    sizeof (CS_SuperBlob) + uCodeSignBlobCount * sizeof (CS_BlobIndex);
    uint32_t uCodeSignLength = uSuperBlobHeaderLength + uCodeDirectorySlotLength +
    uRequirementsSlotLength + uEntitlementsSlotLength + uDerEntitlementsLength +
    uAltnateCodeDirectorySlotLength + uCMSSignatureSlotLength;
    ...
    if (uEntitlementsSlotLength > 0) {
        ...
        blob.offset =
        BE (uSuperBlobHeaderLength + uCodeDirectorySlotLength + uRequirementsSlotLength);
        arrBlobIndexes.push_back (blob);
    }
    if (uDerEntitlementsLength > 0) {
        ...
        blob.offset = BE (uSuperBlobHeaderLength + uCodeDirectorySlotLength +
        uRequirementsSlotLength + uEntitlementsSlotLength);
        arrBlobIndexes.push_back (blob);
    }
    ...
    if (ZLog::IsDebug ()) {
        ...
        WriteFile ("./.zsign_debug/Entitlements.plist.new",
        strEntitlementsSlot.data () + 8, strEntitlementsSlot.size () - 8);
        ...
        WriteFile ("./.zsign_debug/CMSSignature.der.new",
        strCMSSignatureSlot.data () + 8, strCMSSignatureSlot.size () - 8);
...
const string& strCodeResourcesData) {
    ...
    BuildCodeSignature (pSignAsset, bForce, strBundleId, strInfoPlistSHA1,
    strInfoPlistSHA256, strCodeResourcesSHA1, strCodeResourcesSHA256, strCodeSignBlob);
    ...
    if (nSpaceLength < 0) {
        m_bEnoughSpace = false;
        ZLog::WarnV (
        ">>> No Enough CodeSignature Space. Length => Now: %d, Need: %d\n",
        (int)m_uLength - (int)m_uCodeLength, (int)strCodeSignBlob.size ());
        return false;
    }
...
uint32_t ZArchO::ReallocCodeSignSpace (const string& strNewFile) {
    ...
    uint32_t uNewLength = m_uCodeLength +
    ByteAlign (((m_uCodeLength / 4096) + 1) * (20 + 32), 4096) + 16384; // 16K May Be Enough
    ...
    if (NULL == pcslc) {
        ...
        m_pHeader->sizeofcmds =
        BO (BO (m_pHeader->sizeofcmds) + sizeof (codesignature_command));
    }
...
bool ZArchO::InjectDyLib (bool bWeakInject, const char* szDyLibPath, bool& bCreate) {
        ...
        if (LC_LOAD_DYLIB == uLoadType || LC_LOAD_WEAK_DYLIB == uLoadType) {
            ...
            if (0 == strcmp (szDyLib, szDyLibPath)) {
                ...
                (!bWeakInject && (LC_LOAD_DYLIB != uLoadType))) {
                    dlc->cmd =
                    BO ((uint32_t)(bWeakInject ? LC_LOAD_WEAK_DYLIB : LC_LOAD_DYLIB));
    ...
    {
        ZLog::Error (">>> Can't Find Free Space Of LoadCommands For "
    ...
    // add
    dylib_command* dlc =
    (dylib_command*)(m_pBase + m_uHeaderSize + BO (m_pHeader->sizeofcmds));
