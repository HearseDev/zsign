...
bool ZAppBundle::SignFolder(ZSignAsset *pSignAsset, const string &strFolder,
                            ...
                            const string &strDyLibFile, bool bForce,
  ...
  if (!bRemoveProv) {
    if (!WriteFile(pSignAsset->m_strProvisionData,
                   "%s/embedded.mobileprovision",
  ...
  if (!strDyLibFile.empty()) { // inject dylib
    ...
    if (!strDyLibData.empty()) {
      ...
      if (WriteFile(strDyLibData, "%s/%s", m_strAppFolder.c_str(),
                    strFileName.c_str())) {
        StringFormat(m_strDyLibPath, "@executable_path/%s",
                     strFileName.c_str());
      }
    }
  }
