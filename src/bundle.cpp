        ...
        while (NULL != ptr) {
            ...
            0 != strcmp (ptr->d_name, "__MACOSX")) {
                ...
                if (DT_DIR == ptr->d_type) {
                    ...
                    ZLog::DebugV (
                    ">>> Unknown directory entry type for %s, falling back "
...
bool ZAppBundle::SignNode (JValue& jvNode) {
    ...
    if (strBundleId.empty () || strBundleExe.empty () ||
    strInfoPlistSHA1.empty () || strInfoPlistSHA256.empty ()) {
        ZLog::ErrorV (
        ">>> Can't Get BundleID or BundleExecute or Info.plist SHASum "
        "in Info.plist! %s\n",
        strFolder.c_str ());
        return false;
    }
    ...
    if (m_bForceSign || jvCodeRes.isNull ()) { // create
        ...
        for (size_t i = 0; i < jvNode["changed"].size (); i++) {
            ...
            if (!SHASumBase64File (strRealFile.c_str (), strFileSHA1Base64, strFileSHA256Base64)) {
                ZLog::ErrorV (
                ">>> Can't Get Changed File SHASumBase64! %s", strFile.c_str ());
                return false;
            }
...
bool bRemoveProv) {
        ...
        if (jvInfoPlist.readPListPath ("%s/Info.plist", m_strAppFolder.c_str ())) {
            ...
            if (!strBundleID.empty ()) {
                ...
                for (size_t i = 0; i < arrPlugIns.size (); i++) {
                    ...
                    if (jvPlugInInfoPlist.readPListPath (
                        "%s/Info.plist", strPlugin.c_str ())) {
                        ...
                        if (jvPlugInInfoPlist.has (
                            ...
                            jvPlugInInfoPlist["WKCompanionAppBundleIdentifie"
                                              "r"] = strNewWKCBundleID;
                            ZLog::PrintV (
                            ">>> BundleId: \t%s -> %s, "
                        ...
                        if (jvPlugInInfoPlist.has ("NSExtension")) {
                            if (jvPlugInInfoPlist["NSExtension"].has (
                                ...
                                if (jvPlugInInfoPlist["NSExtension"]["NSExtensi"
                                                                     "onAttribu"
                                                                     "tes"]
                                    .has ("WKAppBundleIdentifier")) {
                                    string strOldWKBundleID =
                                    jvPlugInInfoPlist["NSExtension"]
                                                     ["NSExtensionAttributes"]["WKAppBundleIdentifier"];
                                    string strNewWKBundleID = strOldWKBundleID;
                                    StringReplace (strNewWKBundleID,
                                    strOldBundleID, strBundleID);
                                    jvPlugInInfoPlist["NSExtension"]
                                                     ["NSExtensionAttributes"]["WKAppBundleIdentifier"] =
                                                     strNewWKBundleID;
                                    ZLog::PrintV (
                        ...
                        jvPlugInInfoPlist.writePListPath (
                        "%s/Info.plist", strPlugin.c_str ());
                    }
                }
            }
    ...
    if (!strDisplayName.empty ()) {
        ...
        if (jvInfoPlistStrings.readPListPath (
            "%s/zh_CN.lproj/InfoPlist.strings", m_strAppFolder.c_str ())) {
            jvInfoPlistStrings["CFBundleName"]        = strDisplayName;
            jvInfoPlistStrings["CFBundleDisplayName"] = strDisplayName;
            jvInfoPlistStrings.writePListPath (
            "%s/zh_CN.lproj/InfoPlist.strings", m_strAppFolder.c_str ());
        }
        jvInfoPlistStrings.clear ();
        if (jvInfoPlistStrings.readPListPath (
            "%s/zh-Hans.lproj/InfoPlist.strings", m_strAppFolder.c_str ())) {
            jvInfoPlistStrings["CFBundleName"]        = strDisplayName;
            jvInfoPlistStrings["CFBundleDisplayName"] = strDisplayName;
            jvInfoPlistStrings.writePListPath (
            "%s/zh-Hans.lproj/InfoPlist.strings", m_strAppFolder.c_str ());
    ...
    if (m_bForceSign) {
        ...
        if (!GetSignFolderInfo (m_strAppFolder, jvRoot, true)) {
            ZLog::ErrorV (
            ">>> Can't Get BundleID, BundleVersion, or BundleExecute in "
            "Info.plist! %s\n",
