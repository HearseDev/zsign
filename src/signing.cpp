...
bool SlotBuildRequirements (const string& strBundleID, const string& strSubjectCN, string& strOutput) {
    ...
    strPaddedBundleID.append (
    ((strBundleID.size () % 4) ? (4 - (strBundleID.size () % 4)) : 0), 0);

    string strPaddedSubjectID = strSubjectCN;
    strPaddedSubjectID.append (
    ((strSubjectCN.size () % 4) ? (4 - (strSubjectCN.size () % 4)) : 0), 0);

    uint8_t magic1[]  = { 0xfa, 0xde, 0x0c, 0x01 };
    uint32_t uLength1 = 0;
    uint8_t pack1[]   = { 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x03, 0x00,
          0x00, 0x00, 0x14 };
    uint8_t magic2[]  = { 0xfa, 0xde, 0x0c, 0x00 };
    uint32_t uLength2 = 0;
    uint8_t pack2[]   = { 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x06, 0x00,
          0x00, 0x00, 0x02 };
...
bool SlotParseEntitlements (uint8_t* pSlotBase, CS_BlobIndex* pbi) {
    ...
    if (ZLog::IsDebug ()) {
        ...
        WriteFile ("./.zsign_debug/Entitlements.plist",
        (const char*)pSlotBase + 8, uSlotLength - 8);
    }
    return true;
}

bool SlotParseDerEntitlements (uint8_t* pSlotBase, CS_BlobIndex* pbi) {
    uint32_t uSlotLength =
    SlotParseGeneralHeader ("CSSLOT_DER_ENTITLEMENTS", pSlotBase, pbi);
    if (uSlotLength < 8) {
        return false;
    }
...
bool SlotParseCodeDirectory (uint8_t* pSlotBase, CS_BlobIndex* pbi) {
    uint32_t uSlotLength =
    SlotParseGeneralHeader ("CSSLOT_CODEDIRECTORY", pSlotBase, pbi);
    if (uSlotLength < 8) {
        return false;
    }
    ...
    for (int i = LE (cdHeader.nSpecialSlots) - 1; i >= 0; i--) {
        const char* suffix = "\t\n";
        switch (i) {
        case 0: suffix = "\tInfo.plist\n"; break;
        case 1: suffix = "\tRequirements Slot\n"; break;
        case 2: suffix = "\tCodeResources\n"; break;
        case 3: suffix = "\tApplication Specific\n"; break;
        case 4: suffix = "\tEntitlements Slot\n"; break;
        case 6: suffix = "\tEntitlements(DER) Slot\n"; break;
        }
    ...
    if (ZLog::IsDebug ()) {
        if (1 == cdHeader.hashType) {
            WriteFile ("./.zsign_debug/CodeDirectory_SHA1.slot",
            (const char*)pSlotBase, uSlotLength);
        } else if (2 == cdHeader.hashType) {
            WriteFile ("./.zsign_debug/CodeDirectory_SHA256.slot",
            (const char*)pSlotBase, uSlotLength);
        }
    }
...
string& strOutput) {
    strOutput.clear ();
    if (NULL == pCodeBase || uCodeLength <= 0 || strBundleId.empty () ||
    strTeamId.empty ()) {
        return false;
    }
    ...
    if (isExecuteArch) {
        arrSpecialSlots.push_back (
        strDerEntitlementsSlotSHA.empty () ? strEmptySHA : strDerEntitlementsSlotSHA);
        arrSpecialSlots.push_back (strEmptySHA);
    }
    arrSpecialSlots.push_back (
    strEntitlementsSlotSHA.empty () ? strEmptySHA : strEntitlementsSlotSHA);
    arrSpecialSlots.push_back (strEmptySHA);
    arrSpecialSlots.push_back (strCodeResourcesSHA.empty () ? strEmptySHA : strCodeResourcesSHA);
    arrSpecialSlots.push_back (
    strRequirementsSlotSHA.empty () ? strEmptySHA : strRequirementsSlotSHA);
    ...
    uint32_t uSlotLength =
    uHeaderLength + uBundleIDLength + uSpecialSlotsLength + uCodeSlotsLength;
    if (uVersion >= 0x20100) {
        // todo
    }
    if (uVersion >= 0x20200) {
    ...
    if (NULL != pCodeSlotsData &&
    (uCodeSlotsDataLength == uCodeSlots * cdHeader.hashSize)) { // use exists
...
bool SlotParseCMSSignature (uint8_t* pSlotBase, CS_BlobIndex* pbi) {
    uint32_t uSlotLength =
    SlotParseGeneralHeader ("CSSLOT_SIGNATURESLOT", pSlotBase, pbi);
    if (uSlotLength < 8) {
        return false;
    }
    ...
    for (size_t i = 0; i < jvInfo["attrs"]["unknown"].size (); i++) {
        ...
        jvAttr["obj"].asCString (), jvAttr["name"].asCString (),
        jvAttr["type"].asInt (), jvAttr["count"].asInt ());
    }
    ZLog::Print ("\n");
    ...
    if (ZLog::IsDebug ()) {
        ...
        WriteFile ("./.zsign_debug/CMSSignature.der",
        (const char*)pSlotBase + 8, uSlotLength - 8);
    }
    return true;
}
...
string& strOutput) {
    ...
    string strCMSData;
    if (!pSignAsset->GenerateCMS (strCodeDirectorySlot, strCDHashesPlist,
        strCodeDirectorySlotSHA1, strAltnateCodeDirectorySlot256, strCMSData)) {
        return false;
    }
...
bool ParseCodeSignature (uint8_t* pCSBase) {
    ...
    for (uint32_t i = 0; i < LE (psb->count); i++, pbi++) {
        ...
        case CSSLOT_CODEDIRECTORY:
            ...
            break;
        case CSSLOT_REQUIREMENTS: SlotParseRequirements (pSlotBase, pbi); break;
        case CSSLOT_ENTITLEMENTS: SlotParseEntitlements (pSlotBase, pbi); break;
        ...
        case CSSLOT_TICKETSLOT:
            ...
            break;
        default:
            SlotParseGeneralTailer (pSlotBase,
            SlotParseGeneralHeader ("CSSLOT_UNKNOWN", pSlotBase, pbi));
            break;
        }
    }

    if (ZLog::IsDebug ()) {
        WriteFile ("./.zsign_debug/CodeSignature.blob", (const char*)pCSBase,
        LE (psb->length));
    }
    return true;
}
...
uint32_t& uCodeSlots256DataLength) {
    ...
    return ((NULL != pCodeSlots1Data) && (NULL != pCodeSlots256Data) &&
    uCodeSlots1DataLength > 0 && uCodeSlots256DataLength > 0);
}
