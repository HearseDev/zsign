...
void *MapFile(const char *path, size_t offset, size_t size, size_t *psize,
              bool ro) {
  ...
  void *base = mmap(NULL, size, ro ? PROT_READ : PROT_READ | PROT_WRITE,
...
void PrintSHASum(const char *prefix, const uint8_t *hash, uint32_t size,
                 const char *suffix) {
...
void PrintSHASum(const char *prefix, const string &strSHASum,
                 const char *suffix) {
  PrintSHASum(prefix, (const uint8_t *)strSHASum.data(), strSHASum.size(),
              suffix);
}

void PrintDataSHASum(const char *prefix, int nSumType, const string &strData,
                     const char *suffix) {
  string strSHASum;
...
void PrintDataSHASum(const char *prefix, int nSumType, uint8_t *data,
...
bool SHASumBase64(const string &strData, string &strSHA1Base64,
...
bool SHASumBase64File(const char *szFile, string &strSHA1Base64,
                      string &strSHA256Base64) {
  ZBase64 b64;
  string strSHA1;
  string strSHA256;
