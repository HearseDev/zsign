...
bool JValue::join(JValue &jv) {
  if ((E_OBJECT == m_eType || E_NULL == m_eType) && E_OBJECT == jv.type()) {
    ...
    return true;
  } else if ((E_ARRAY == m_eType || E_NULL == m_eType) &&
             E_ARRAY == jv.type()) {
...
bool JValue::readPList(const char *pdoc, size_t len /*= 0*/,
                       string *pstrerr /*= NULL*/) {
  if (NULL == pdoc) {
    return false;
  }
...
bool JReader::decodeNumber(Token &token, JValue &jval) {
  ...
  for (const char *p = pcur; p != token.pend; p++) {
    ...
    if ('.' == c || 'e' == c || 'E' == c) {
      ...
      return addError("'" + string(token.pbeg, token.pend) +
                          "' is not a number.",
                      token.pbeg);
    } else {
      val = val * 10 + (c - '0');
...
string JWriter::d2s(time_t t) {
  ...
  snprintf(szDate, sizeof(szDate), "%04d-%02d-%02dT%02d:%02d:%02dZ",
           ft.tm_year + 1900, ft.tm_mon + 1, ft.tm_mday, ft.tm_hour, ft.tm_min,
           ft.tm_sec);
  return szDate;
}
...
bool PReader::readValue(JValue &pval, Token &token) {
  ...
  default:
    return addError("Syntax error: value, dictionary or array expected.",
                    token.pbeg);
    break;
  }
  return true;
...
bool PReader::readToken(Token &token) {
  ...
  } else if ("<data/>" == label || "<date/>" == label || "<string/>" == label ||
             "<integer/>" == label || "<real/>" == label) {
...
bool PReader::readBinaryValue(const char *&pcur, JValue &pv) {
  ...
  case BPLIST_UNK_0x70: {
    ...
    for (size_t i = 0; i < size; i++) {
      uint64_t uIndex = getUIntVal((const char *)pcur + i * m_uDictParamSize,
                                   m_uDictParamSize);
      if (uIndex < m_uObjects) {
        const char *pval =
            (m_pBeg + getUIntVal(m_pOffsetTable + uIndex * m_uOffsetSize,
                                 m_uOffsetSize));
  ...
  case BPLIST_DICT: {
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      uint64_t uKeyIndex = getUIntVal((const char *)pcur + i * m_uDictParamSize,
                                      m_uDictParamSize);
      ...
      if (uKeyIndex < m_uObjects) {
        const char *pval =
            (m_pBeg + getUIntVal(m_pOffsetTable + uKeyIndex * m_uOffsetSize,
                                 m_uOffsetSize));
      ...
      if (uValIndex < m_uObjects) {
        const char *pval =
            (m_pBeg + getUIntVal(m_pOffsetTable + uValIndex * m_uOffsetSize,
                                 m_uOffsetSize));
...
void PWriter::FastWriteValue(const JValue &pval, string &strdoc,
                             string &strindent) {
  if (pval.isObject()) {
    strdoc += strindent;
    if (pval.isEmpty()) {
...
string &PWriter::StringReplace(string &context, const string &from,
                               const string &to) {
  size_t lookHere = 0;
  size_t foundHere;
